...
class SemanticMemory:
    ...
    def list_all(self, limit: int = 500) -> List["Knowledge"]:
        """
        Full listing of stored facts, newest-updated first -- distinct
        from semantic_search()/hybrid_search(), which only return
        items matching a query. Added for the web dashboard's memory
        browser (GET /api/memory/engrams), which needs to show
        everything JARVIS knows, not a search result subset.
        """
        with self._lock, self._get_db_connection() as conn:
            cursor = conn.execute(
    ...
    def semantic_search(
        ...
        max_candidate_cap: int = 20
    ) -> List[Knowledge]:
        """
        Empirical Engram Activation Search.
        Threshold S_cos >= similarity_threshold (e.g. 0.70) par dynamic retrieval karti hai.
        """
        with self._lock:
            ...
            for dist, idx in zip(distances[0], indices[0]):
                ...
                if idx != -1 and idx in self.faiss_idx_to_id:
                    # L2 Squared distance to Cosine Similarity transformation
    ...
    # core/memory/semantic_memory.py me is tarah update karein:
    def hybrid_search(
        self,
        query: str,
        limit: int = 5,
    # ... baki code same rahega
        similarity_threshold: float = 0.70, 
        lexical_fallback_limit: int = 5
    ) -> List[Knowledge]:
        """
        Semantic similarity threshold pass hone wale facts prioritize hote hain, 
        aur zero activations ki sthiti mein lexical search fallback execute hota hai.
        """
        semantic_results = self.semantic_search(query, similarity_threshold=similarity_threshold)
        
        # Agar threshold matching facts kam milte hain tabhi lexical fallback execute hoga
    ...
    def get_trimmed_context(
        ...
        Context budget activation score ke proportionally dynamic expand/contract hota hai.
        """
        # 1. Active Vector Facts Filtered by Mathematical Threshold
        ...
        # 2. Graph Relations for Active Subject
        if subject:
            # Dynamic graph limit based on active semantic density
            graph_limit = max(3, len(facts) * 2)
            relations = self.get_graph_relations(subject, max_limit=graph_limit)
            for r in relations:
                context_lines.append(f"Relation: {r['subject']} -> {r['predicate']} -> {r['target']}")

        return "\n".join(context_lines)
     
    # NEW (Max 5 Graph Relations Limit):  
    ...
    def snapshot(
        ...
        with self._lock:
            ...
            knowledge_items = [
                Knowledge.from_dict(dict(row)).to_dict()
                for row in rows
            ]
    ...
    def restore(
        ...
        for data in raw_knowledge:
            ...
            try:
                ...
                print(
                    f"[SemanticMemory] Restore warning: {exc}"
                )
