...
def resolve_dependencies(missing: List[str]) -> Dict[str, Any]:
    """Decide what to do about missing imports. Installs ONLY what UK
    has already allowlisted; everything else is reported."""
    missing = [m.strip().lower() for m in (missing or []) if m and m.strip()]
    if not missing:
        return {"installed": [], "refused": [], "note": None}

    allow = set(allowed_packages())
    ...
    for pkg in missing:
        if not _SAFE_PACKAGE_NAME.match(pkg):
            ...
            continue
        if not enabled:
            refused.append({"package": pkg, "reason": "JARVIS_ALLOW_INSTALL .env mein enabled nahi hai."})
            continue
        if pkg not in allow:
            refused.append({"package": pkg, "reason": "Allowlist mein nahi hai."})
            continue
        try:
            proc = subprocess.run(
            ...
            if proc.returncode == 0:
                ...
                log_event("sandbox", f"installed allowlisted package: {pkg}", level="info")
            else:
    ...
    if refused:
        ...
        note = (
            f"Yeh packages install nahi kiye: {names}. "
            f"Agar inhe allow karna hai toh .env mein yeh line rakho:\n"
            f"  JARVIS_ALLOW_INSTALL=true\n"
            f"  JARVIS_ALLOWED_PACKAGES={','.join(sorted(set(allowed_packages()) | {r['package'] for r in refused}))}\n"
            f"Allowlist isliye hai kyunki pip install ke waqt package ka apna setup.py chalta hai -- "
            f"matlab galat ya typo-squatted naam install karte hi code run kar deta hai, import se pehle."
        )
    return {"installed": installed, "refused": refused, "note": note}
