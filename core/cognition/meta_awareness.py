...
def _is_workflow_query(text: str) -> bool:
    ...
    return (
        _fuzzy_has_any(words, _META_REFERENCE_WORDS)
        and _fuzzy_has_any(words, _META_SUBJECT_WORDS)
...
def try_workflow_narration_answer(user_input: str, brain: Any = None) -> Optional[str]:
    ...
    try:
        from ..runtime.trace_log import get_trace_log
        recent = get_trace_log().read_recent(limit=1)
    except Exception:
        recent = []
    ...
    if not entry:
        return "Abhi tak koi pichhla turn trace log mein record nahi hai."

    perception = entry.get("perception") or {}
    source = perception.get("source", "unknown") if isinstance(perception, dict) else "unknown"
    confidence = perception.get("confidence") if isinstance(perception, dict) else None
    grounded = entry.get("stayed_within_brief")
    budget = entry.get("llm_budget") or {}

    parts = [f'Pichhla turn: aapne kaha "{entry.get("user_input", "?")}".']
    parts.append(f"Perception source={source}" + (f", confidence={confidence:.2f}" if isinstance(confidence, (int, float)) else "") + ".")
    parts.append(f"Mode={entry.get('mode', '?')}, status={entry.get('status', '?')}.")
    if grounded is True:
        parts.append("Response grounded tha (brief ke bahar kuch nahi kaha).")
    elif grounded is False:
        parts.append(f"Response FLAGGED hua tha: {entry.get('flagged_unsupported')}.")
    if budget:
        parts.append(f"LLM calls use hui: {budget.get('calls', '?')}/{budget.get('max_calls', '?')}.")
    return " ".join(parts)
