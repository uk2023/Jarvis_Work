...
def resolve_mode(mode: str, user_input: str,
    ...
    """Turn the user's setting (off/on/auto) into a decision for THIS turn."""
    mode = (mode or MODE_OFF).strip().lower()
    if mode == MODE_ON:
        return {"think": True, "mode": MODE_ON, "reason": "Extended thinking aapne on kiya hua hai."}
    if mode == MODE_AUTO:
        d = should_think_harder(user_input, information_need)
        return {"think": d["think"], "mode": MODE_AUTO, "reason": d["reason"]}
    return {"think": False, "mode": MODE_OFF, "reason": "Extended thinking off hai."}
...
def think_through(generate: Callable, *, user_input: str, mode: str = MODE_AUTO,
