...
class BlueprintBrain(Brain):
    ...
    def think_and_respond(self, user_input: str, identity_profile: Optional[Dict[str, Any]] = None, source: str = "cli") -> str:
        ...
        if self.llm is not None:
            ...
            if callable(begin_budget):
                begin_budget()
        try:
            response = super().think_and_respond(user_input=user_input, identity_profile=identity_profile, source=source)
            ...
            self._enqueue_learning(event_type="USER_CHAT", context={"user_input": str(user_input or ""), "semantic": semantic, "cognition": cognition}, action={"decision": decision, "action_response": action_response if isinstance(action_response, dict) else {}}, outcome={"response": str(response), "action": action_response if isinstance(action_response, dict) else {}}, source=source, importance=0.5)
            return response
        finally:
    ...
    def _perceive(self, user_input: str) -> Dict[str, Any]:
        ...
        integrated = self.semantic_understanding.understand(semantic_input["perception"]["normalized_input"], context=semantic_input["semantic_context"], retrieve=True)
