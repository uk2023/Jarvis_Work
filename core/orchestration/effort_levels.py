...
EFFORT_PROFILES: Dict[str, EffortProfile] = {
    ...
    "low": EffortProfile(
        ...
        speed_note="Sabse tez -- turns le sakta hai (18 tak), verify retry nahi, steps chhupe rehte hain.",
    ),
    "medium": EffortProfile(
        name="medium", label="Medium",
        thinking_stages=1, max_task_steps=35, verify_retries=1,
        step_visibility="summary", min_budget_calls=2,
        speed_note="20-40 step tak, thodi soch (samajhna), ek verify-retry. Default.",
    ),
    "high": EffortProfile(
        name="high", label="High",
        thinking_stages=2, max_task_steps=55, verify_retries=2,
        step_visibility="summary", min_budget_calls=3,
        speed_note="Samajhna + options explore, do verify-retry, 55 step tak.",
    ),
    "aggressive": EffortProfile(
        name="aggressive", label="Aggressive",
        thinking_stages=3, max_task_steps=80, verify_retries=3,
        step_visibility="full", min_budget_calls=4,
        speed_note="Poora thinking cycle, 80 step tak, steps poore dikhte hain.",
    ),
    "deep": EffortProfile(
        name="deep", label="Deep",
        thinking_stages=3, max_task_steps=100, verify_retries=4,
        step_visibility="full", min_budget_calls=5,
        speed_note="Sabse dheema -- poora thinking, 100 step tak, sab explicit tareeke se dikhta hai.",
    ),
}
