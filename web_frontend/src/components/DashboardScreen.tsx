...
export function DashboardScreen({ onSelectTurn, theme, activeSection = 'monitor' }: DashboardScreenProps) {
  ...
  const tableSurface = isDark ? '!bg-[#0f141d] !border-[#293446] !text-[#edf3fb]' : '!bg-white !border-slate-200 !text-slate-900';
  const tableHeader = isDark ? 'bg-[#151c27] text-[#9aa8ba]' : 'bg-slate-50 text-slate-600';
  const tableRow = isDark ? 'hover:bg-white/[.025]' : 'hover:bg-slate-50';
  const cellBorder = isDark ? 'border-[#293446]' : 'border-slate-200';
  const muted = isDark ? 'text-[#9aa8ba]' : 'text-slate-500';
  const bodyText = isDark ? 'text-[#edf3fb]' : 'text-slate-700';
  ...
  // Both tables intentionally share exactly the same three-column geometry.
  // Timestamp + secondary field stay compact; the content/message column gets the remaining width.
  const tableGrid = 'grid-cols-[68px_82px_minmax(0,1fr)]';
  const tableBase = `border rounded-lg overflow-hidden ${tableSurface}`;
  ...
  const activityTable = (expanded = false) => <div className={`${tableBase} ${expanded ? 'min-w-[760px]' : 'w-full'}`}>
    <div className={`grid ${tableGrid} gap-0 border-b ${cellBorder} ${tableHeader} text-[7px] uppercase tracking-[.14em]`}>
      <span className={`flex items-center justify-center border-r ${cellBorder} px-1.5 py-1.5 text-center`}>Timestamp</span>
      <span className={`flex items-center justify-center border-r ${cellBorder} px-1.5 py-1.5 text-center`}>Transaction</span>
      <span className="flex items-center justify-center px-2 py-1.5 text-center">Content</span>
    </div>
    <div className={expanded ? '' : 'max-h-44 overflow-y-auto'}>
      {history.length === 0 ? <div className={`px-2.5 py-3 ${muted} text-[9px]`}>No activity history available.</div> : history.map(item => {
        const selected = selectedTurnId === item.turnId;
        return <div key={item.turnId} onClick={() => { setSelectedTurnId(item.turnId); onSelectTurn(item.turnId); }} className={`group relative grid ${tableGrid} items-center gap-0 cursor-pointer border-b last:border-0 ${cellBorder} ${tableRow} text-[8px] transition-all ${selected ? (isDark ? 'bg-[#5b8def]/12' : 'bg-[#5b8def]/[.08]') : ''}`}>
          <span className={`border-r ${cellBorder} px-1.5 py-2 tabular-nums ${muted} whitespace-nowrap`}>{formatTime(item.startTime)}</span>
          <span className={`min-w-0 truncate border-r ${cellBorder} px-1.5 py-2 font-bold text-[#5b8def]`} title={item.turnId}>{item.turnId}</span>
          <span className={`min-w-0 break-words px-2 py-2 pr-8 ${bodyText}`} title={item.query}>{item.query || '—'}</span>
  ...
  const logsTable = (expanded = false) => <div className={`${tableBase} ${expanded ? 'min-w-[760px]' : 'w-full'}`}>
    <div className={`grid ${tableGrid} gap-0 border-b ${cellBorder} ${tableHeader} text-[7px] uppercase tracking-[.14em]`}>
      <span className={`flex items-center justify-center border-r ${cellBorder} px-1.5 py-1.5 text-center`}>Timestamp</span>
      <span className={`flex items-center justify-center border-r ${cellBorder} px-1.5 py-1.5 text-center`}>Level</span>
      <span className="flex items-center justify-center px-2 py-1.5 text-center">Diagnostic Message</span>
    </div>
    <div className={expanded ? '' : 'max-h-44 overflow-y-auto'}>
      {normalizedLogs.length === 0 ? <div className={`px-2.5 py-3 ${muted} text-[9px]`}>No diagnostic logs available.</div> : normalizedLogs.map((log, index) => {
        const warning = log.level === 'warning' || log.level === 'warn';
        const error = log.level === 'error';
        return <div key={`${log.timestamp}-${index}`} className={`grid ${tableGrid} items-start gap-0 border-b last:border-0 ${cellBorder} ${tableRow} text-[8px] leading-relaxed min-w-0 ${error ? 'text-rose-500' : warning ? 'text-amber-500' : bodyText}`}>
          <span className={`border-r ${cellBorder} px-1.5 py-2 tabular-nums ${muted} whitespace-nowrap`}>{formatTime(log.timestamp)}</span>
          <span className={`border-r ${cellBorder} px-1.5 py-2 font-bold uppercase ${error ? 'text-rose-500' : warning ? 'text-amber-500' : 'text-[#5b8def]'}`}>{log.level}</span>
          <span className="min-w-0 break-words px-2 py-2">{log.message}</span>
        </div>;
      })}
    </div>
  </div>;
  ...
  return <div className={`h-full overflow-y-auto overflow-x-hidden p-3 sm:p-5 space-y-3 font-mono text-xs max-w-full ${isDark ? 'trace-dark' : 'trace-light'}`}>
    {showMonitor && <>
      ...
      <section id="card-activity-history" className={`overflow-hidden ${surface}`}><div className="px-3 py-2.5 sm:px-4"><div className="flex items-center justify-between gap-3 mb-2"><div className="flex items-center gap-2 min-w-0"><BarChart3 className="w-3.5 h-3.5 text-[#5b8def] shrink-0" /><div className="min-w-0"><h2 className="font-bold text-[9px] uppercase tracking-wider truncate">Recent Activity History</h2><div className={`text-[7px] ${muted} truncate`}>Transaction stream · select a row to inspect</div></div></div><div className="flex items-center gap-2 shrink-0"><span className={`text-[7px] ${muted} uppercase tracking-[.1em]`}>{history.length} turns</span>{expandButton('activity')}</div></div>{activityTable()}</div></section>
      <section id="card-internal-logs" className={`overflow-hidden ${surface}`}><div className="px-3 py-2.5 sm:px-4"><div className="flex items-center justify-between gap-3 mb-2"><div className="flex items-center gap-2 min-w-0"><Terminal className="w-3.5 h-3.5 text-[#5b8def] shrink-0" /><div className="min-w-0"><h2 className="font-bold text-[9px] uppercase tracking-wider truncate">Internal Diagnostic Logs</h2><div className={`text-[7px] ${muted} truncate`}>Runtime diagnostics · warnings and errors surfaced inline</div></div></div>{expandButton('logs')}</div>{logsTable()}</div></section>
    </>}
    ...
    {expandedTable && <div className="fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm p-2 sm:p-5 flex items-center justify-center"><div className={`w-full h-full max-w-[1500px] flex flex-col rounded-xl border shadow-2xl overflow-hidden ${isDark ? 'bg-[#0f141d] border-[#293446] text-[#edf3fb]' : 'bg-white border-slate-200 text-slate-900'}`}><div className={`flex items-center justify-between gap-3 px-3 sm:px-5 py-3 border-b ${cellBorder} shrink-0 ${isDark ? 'bg-[#151c27]' : 'bg-slate-50'}`}><div><h2 className="font-black text-[10px] sm:text-xs uppercase tracking-[.14em]">{expandedTable === 'activity' ? 'Recent Activity History' : 'Internal Diagnostic Logs'}</h2><p className={`mt-1 text-[8px] ${muted}`}>Full-window diagnostic table · spreadsheet view</p></div><button onClick={() => setExpandedTable(null)} className={`p-2 rounded-lg border ${cellBorder} ${isDark ? 'bg-white/[.04]' : 'bg-white'} hover:opacity-80`} title="Close" aria-label="Close"><X className="w-4 h-4" /></button></div><div className="flex-1 overflow-auto p-3 sm:p-5">{expandedTable === 'activity' ? activityTable(true) : logsTable(true)}</div></div></div>}
  </div>;
}
