...
export function DashboardScreen({ onSelectTurn, onNavigateToCLI, theme, activeSection = 'monitor' }: DashboardScreenProps) {
  ...
  return <div className={`h-full overflow-y-auto overflow-x-hidden p-3 sm:p-5 space-y-3 font-mono text-xs max-w-full ${isDark?'trace-dark':'trace-light'}`}>
   {showMonitor&&<>
    ...
    <section id="card-activity-history" className={`trace-root overflow-hidden ${surface}`}><div className="px-3 py-2 sm:px-3"><div className="flex items-center justify-between gap-3 mb-1.5"><div className="flex items-center gap-2 min-w-0"><BarChart3 className="w-3.5 h-3.5 text-[#5b8def] shrink-0"/><h2 className="font-bold text-[9px] uppercase tracking-wider truncate">Recent Activity History</h2></div><span className="text-[7px] text-slate-500 uppercase shrink-0">Completed Turns</span></div><div className={`border rounded-md overflow-hidden ${logSurface}`}><div className="max-h-44 overflow-y-auto p-1">{history.length===0?<div className="p-2 text-slate-500 text-[9px]">No activity history available.</div>:history.map(item=>{const running=item.endTime===null;return <div key={item.turnId} onClick={()=>onSelectTurn(item.turnId)} className={`group grid grid-cols-[auto_minmax(0,1fr)_auto_minmax(0,1.2fr)_auto] items-center gap-2 px-2 py-1 cursor-pointer border-b last:border-0 border-white/5 text-[8px] transition ${isDark?'hover:bg-white/[.035]':'hover:bg-white/10'}`}><span className="text-[#5b8def] font-bold shrink-0">{item.turnId}</span><span className="truncate text-slate-300" title={item.query}>{item.query}</span><span className={running?'text-amber-400':'text-slate-400'}>{running?'RUNNING':`${item.durationSeconds.toFixed(2)}s`}</span><span className="flex items-center gap-1 min-w-0 truncate text-slate-500">{item.stagePath.map((stage,index)=><React.Fragment key={`${item.turnId}-${stage}-${index}`}><span className="truncate">{stage}</span>{index<item.stagePath.length-1&&<ArrowRight className="w-2 h-2 shrink-0"/>}</React.Fragment>)}</span><button onClick={e=>{e.stopPropagation();onSelectTurn(item.turnId);}} className="text-[#5b8def] opacity-70 group-hover:opacity-100 shrink-0" title="Inspect turn"><ExternalLink className="w-2.5 h-2.5"/></button></div>})}</div></div></div></section>
    <section id="card-internal-logs" className={`trace-root overflow-hidden ${surface}`}><div className="px-3 py-2 sm:px-3"><div className="flex items-center justify-between gap-3 mb-1.5"><div className="flex items-center gap-2 min-w-0"><Terminal className="w-3.5 h-3.5 text-[#5b8def] shrink-0"/><h2 className="font-bold text-[9px] uppercase tracking-wider truncate">Internal Diagnostic Logs</h2></div><button onClick={onNavigateToCLI} className={`px-2 py-1 border rounded-md flex items-center gap-1 text-[8px] font-bold shrink-0 ${isDark?'bg-white/5 hover:bg-white/10 border-white/10 text-[#5b8def]':'bg-slate-50 hover:bg-slate-100 border-slate-200 text-[#4f7ed8]'}`}><Terminal className="w-2.5 h-2.5"/>Open Virtual CLI</button></div><div className={`border rounded-md overflow-hidden ${logSurface}`}><div className="max-h-44 overflow-y-auto p-1.5 space-y-0.5">{(liveState?.logs??[]).length===0?<div className="text-slate-500 text-[9px]">No diagnostic logs available.</div>:(liveState?.logs??[]).map((log,index)=>{const level=typeof log==='object'&&log!==null&&'level' in log?String((log as {level?:string}).level||'info').toLowerCase():'info';const message=typeof log==='string'?log:typeof log==='object'&&log!==null&&'message' in log?String((log as {message?:unknown}).message??''):String(log);const warning=level==='warning'||level==='warn',error=level==='error';return <div key={index} className={`grid grid-cols-[auto_auto_minmax(0,1fr)] gap-1.5 text-[8px] leading-relaxed min-w-0 ${error?'text-rose-400':warning?'text-amber-300':isDark?'text-slate-400':'text-slate-300'}`}><span className="opacity-35 shrink-0">[{String(index+1).padStart(3,'0')}]</span><span className={`font-bold uppercase shrink-0 ${error?'text-rose-400':warning?'text-amber-400':'text-[#5b8def]'}`}>{level}</span><span className="min-w-0 break-words">{message}</span></div>})}</div></div></div></section>
   </>}
