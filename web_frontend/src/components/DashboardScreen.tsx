...
export function DashboardScreen({ onSelectTurn, theme, activeSection = 'monitor' }: DashboardScreenProps) {
  ...
  const activityGrid = 'grid-cols-[86px_140px_minmax(0,1fr)]';
  const logsGrid = activityGrid;
  const tableBase = `border rounded-lg overflow-hidden ${tableSurface}`;
  const activityTable=(expanded=false)=><div className={`${tableBase} ${expanded?'min-w-[760px]':''}`}><div className={`grid ${activityGrid} gap-0 border-b ${cellBorder} ${tableHeader} text-[7px] uppercase tracking-[.14em]`}><span className={`flex items-center justify-center border-r ${cellBorder} px-2.5 py-1.5 text-center`}>Timestamp</span><span className={`flex items-center justify-center border-r ${cellBorder} px-2.5 py-1.5 text-center`}>Transaction</span><span className="flex items-center justify-center px-2.5 py-1.5 text-center">Content</span></div><div className={expanded?'':'max-h-44 overflow-y-auto'}>{history.length===0?<div className={`px-2.5 py-3 ${muted} text-[9px]`}>No activity history available.</div>:history.map(item=>{const selected=selectedTurnId===item.turnId;return <div key={item.turnId} onClick={()=>{setSelectedTurnId(item.turnId);onSelectTurn(item.turnId);}} className={`group relative grid ${activityGrid} items-center gap-0 cursor-pointer border-b last:border-0 ${cellBorder} ${tableRow} text-[8px] transition-all ${selected?(isDark?'bg-[#5b8def]/12':'bg-[#5b8def]/[.08]'):''}`}><span className={`border-r ${cellBorder} px-2.5 py-2 tabular-nums ${muted}`}>{formatTime(item.startTime)}</span><span className={`min-w-0 truncate border-r ${cellBorder} px-2.5 py-2 font-bold text-[#5b8def]`} title={item.turnId}>{item.turnId}</span><span className={`min-w-0 truncate px-2.5 py-2 ${bodyText}`} title={item.query}>{item.query||'—'}</span>{selected&&<span className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#5b8def]"/>}<button onClick={e=>{e.stopPropagation();setSelectedTurnId(item.turnId);onSelectTurn(item.turnId);}} className={`absolute right-1.5 p-1 rounded ${isDark?'bg-black/30':'bg-white'} opacity-0 group-hover:opacity-100 text-[#5b8def] transition`} title="Inspect transaction" aria-label="Inspect transaction"><ExternalLink className="w-2.5 h-2.5"/></button></div>;})}</div></div>;
  const logsTable=(expanded=false)=><div className={`${tableBase} ${expanded?'min-w-[760px]':''}`}><div className={`grid ${logsGrid} gap-0 border-b ${cellBorder} ${tableHeader} text-[7px] uppercase tracking-[.14em]`}><span className={`flex items-center justify-center border-r ${cellBorder} px-2.5 py-1.5 text-center`}>Timestamp</span><span className={`flex items-center justify-center border-r ${cellBorder} px-2.5 py-1.5 text-center`}>Level</span><span className="flex items-center justify-center px-2.5 py-1.5 text-center">Diagnostic Message</span></div><div className={expanded?'':'max-h-44 overflow-y-auto'}>{(liveState?.logs??[]).length===0?<div className={`px-2.5 py-3 ${muted} text-[9px]`}>No diagnostic logs available.</div>:(liveState?.logs??[]).map((log,index)=>{const level=typeof log==='object'&&log!==null&&'level' in log?String((log as {level?:string}).level||'info').toLowerCase():'info';const message=typeof log==='string'?log:typeof log==='object'&&log!==null&&'message' in log?String((log as {message?:unknown}).message??''):String(log);const timestamp=typeof log==='object'&&log!==null&&'timestamp' in log?Number((log as {timestamp?:number}).timestamp||0):0;const warning=level==='warning'||level==='warn',error=level==='error';return <div key={index} className={`grid ${logsGrid} items-start gap-0 border-b last:border-0 ${cellBorder} ${tableRow} text-[8px] leading-relaxed min-w-0 ${error?'text-rose-500':warning?'text-amber-500':bodyText}`}><span className={`border-r ${cellBorder} px-2.5 py-2 tabular-nums ${muted}`}>{formatTime(timestamp)}</span><span className={`border-r ${cellBorder} px-2.5 py-2 font-bold uppercase ${error?'text-rose-500':warning?'text-amber-500':'text-[#5b8def]'}`}>{level}</span><span className="min-w-0 break-words px-2.5 py-2">{message}</span></div>;})}</div></div>;
