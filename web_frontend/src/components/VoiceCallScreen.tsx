...
export default function VoiceCallScreen({ onClose, onExchange }: Props) {
  ...
  const startCall = () => {
    ...
    const session = startRecognition({
      ...
      onInterim: (text) => {
        // GENUINE INTERRUPT TEST. Two checks, both must pass before
        // this counts as UK actually talking over JARVIS rather than
        // JARVIS's own speaker output leaking into the mic:
        //   1. More than one short word -- a lone syllable is far more
        //      likely to be an echo fragment than an intentional
        //      interruption.
        //   2. The mic's OWN level clears a real-speech floor at this
        //      instant -- phone-speaker-into-mic echo is typically
        //      quieter than a person speaking directly into the mic,
        //      even with no hardware cancellation.
        const words = text.trim().split(/\s+/).filter(Boolean);
        const genuineInterrupt = words.length >= BARGE_IN_MIN_WORDS && levelRef.current > BARGE_IN_LEVEL_FLOOR;

        if (phaseRef.current === 'speaking') {
          if (genuineInterrupt) {
            // Real interruption: stop JARVIS, discard whatever the
            // buffer holds (it may be contaminated by echo captured
            // before this instant), and start listening fresh for
            // what UK actually wants to say next.
            stopSpeaking();
          ...
          // Not a genuine interrupt while speaking: show NOTHING. UK's
          // own spec -- "mic capturing on rahe, par data na sune jab
          // tak khalbali na ho" -- means the leaked echo text should
          // not even appear on screen, let alone be acted on.
          return;
        }
