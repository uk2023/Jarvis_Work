    ...
    >
      ...
      >
        {!hasMessages ? (
          ...
          <div className="max-w-3xl mx-auto space-y-4 w-full pb-2">
            ...
            <div
              className={`flex items-center justify-between pb-2 border-b text-xs font-mono ${
              ...
              {onClearChat && (
                <button
                  ...
                  className={`flex items-center gap-1 transition px-2.5 py-1 rounded-lg cursor-pointer ${
                    isDark
                      ? 'hover:text-brand-300 hover:bg-white/5'
                      : 'hover:text-brand-600 hover:bg-slate-100 text-slate-500'
                  }`}
              ...
              return (
                ...
                >
                  ...
                  {isJarvis && (
                    <div
                      className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 mt-0.5 shadow-sm ${
                        isDark
                          ? 'bg-brand-950/70 border border-brand-400/40 text-brand-300 shadow-[0_0_10px_rgba(34,211,238,0.2)]'
                          : 'bg-slate-900 border border-slate-700 text-brand-300'
                      }`}
                    >
                  ...
                  >
                    {/* Message Card */}
                    <div
                      className={`p-3.5 sm:p-4 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-sm font-sans break-words ${
                        isJarvis
                          ? isDark
                            ? 'bg-[#0f121d] border border-white/10 text-white/90 rounded-tl-sm'
                            : 'bg-white border border-slate-200 text-slate-800 rounded-tl-sm shadow-sm'
                          : isDark
                          ? 'bg-[#152336] border border-brand-500/40 text-brand-50 rounded-tr-sm shadow-[0_0_15px_rgba(34,211,238,0.1)]'
                    ...
                    >
                      ...
                      {msg.extractedFact && (
                        <div
                          className={`mt-2.5 pt-2 border-t flex items-start gap-1.5 text-xs font-mono p-2 rounded-xl border ${
                            isDark
                              ? 'border-white/10 bg-brand-400/10 text-brand-200 border-brand-400/30'
                      ...
                      {isJarvis && (
                        <div
                          className={`mt-2.5 flex items-center justify-between text-xs pt-1.5 border-t font-mono ${
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`p-1 rounded-lg transition cursor-pointer ${
                              isDark
                                ? 'hover:text-brand-300 hover:bg-white/10'
                    ...
                    {isJarvis && trace && (
                      <div
                        className={`w-full border rounded-xl overflow-hidden font-mono text-xs shadow-sm ${
                      ...
                      >
                        <button
                          ...
                          className={`w-full flex items-center justify-between px-3 py-1.5 transition cursor-pointer ${
                            isDark
                              ? 'bg-white/[0.02] hover:bg-white/[0.06] text-brand-300'
                              : 'bg-white hover:bg-slate-100 text-brand-700'
                          }`}
                        >
                          <div className="flex items-center gap-1.5 truncate">
                            <Cpu className="w-3 h-3 text-brand-400 shrink-0" />
                            <span className="font-semibold uppercase tracking-wider text-xs sm:text-xs">
                              Cognitive Trace
                            </span>
                            <span className={isDark ? 'text-white/40 text-xs' : 'text-slate-400 text-xs'}>
                        ...
                        <AnimatePresence>
                          {isTraceOpen && (
                            <motion.div
                              ...
                              className={`p-3 space-y-2 border-t text-xs sm:text-xs ${
                                isDark
                            ...
                            >
                              ...
                              >
                                <div className="flex items-center gap-1 truncate">
                                  <Database className="w-2.5 h-2.5 text-brand-400 shrink-0" />
                                  <span>FAISS: {trace.memoryLookupSeconds.toFixed(3)}s</span>
                                </div>
                                <div className="flex items-center gap-1 truncate">
                                  <Clock className="w-2.5 h-2.5 text-green-400 shrink-0" />
                                  <span>Inference: {trace.llmInferenceSeconds.toFixed(2)}s</span>
                                </div>
                              </div>
                              ...
                              {/* Vector Matches */}
                              <div>
                                <div className="text-brand-500 dark:text-brand-300 font-bold mb-0.5 flex items-center gap-1">
                                  <Database className="w-2.5 h-2.5" /> Vector Matches ({trace.vectorMatches.length}):
                                </div>
                                {trace.vectorMatches.length > 0 ? (
                                  <ul className="space-y-0.5 pl-2.5 border-l border-brand-400/40">
                              ...
                              {/* Footer trace state */}
                              <div
                                className={`pt-1 flex items-center justify-between text-xs sm:text-xs ${
              ...
              >
                <div
                  className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 ${
                    isDark
                      ? 'bg-brand-950/70 border border-brand-400/40 text-brand-300'
                ...
                <div
                  className={`px-3.5 py-2.5 rounded-2xl shadow-sm flex items-center gap-2.5 ${
                    isDark
                      ? 'bg-[#0f121d] border border-white/10 text-brand-200'
                ...
                >
                  <div className="flex gap-1">
                    ...
                    <span className="w-1.5 h-1.5 rounded-full bg-brand-200 animate-bounce" style={{ animationDelay: '300ms' }} />
                  </div>
                  <span className="text-xs sm:text-xs font-mono">
      ...
      >
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto flex flex-col gap-1.5">
          ...
          <div className="flex items-end gap-2 w-full">
            ...
            <div
              className={`flex-1 flex items-end rounded-2xl sm:rounded-3xl px-2.5 py-1.5 transition-all shadow-sm ${
                isDark
                  ? 'bg-white/[0.06] border border-white/15 focus-within:border-brand-400/70 focus-within:shadow-[0_0_15px_rgba(34,211,238,0.15)]'
                  : 'bg-slate-100 border border-slate-300 focus-within:border-brand-500 focus-within:bg-white focus-within:shadow-md'
              }`}
            >
              ...
              <div className="flex items-center gap-0.5 shrink-0 self-end mb-0.5">
                ...
                <button
                  ...
                  className={`w-8 h-8 rounded-xl flex items-center justify-center transition cursor-pointer ${
                    isDark
                      ? 'text-white/50 hover:text-brand-300 hover:bg-white/10'
                      : 'text-slate-500 hover:text-brand-600 hover:bg-slate-200'
                  }`}
              ...
              <textarea
                ...
                className={`flex-1 bg-transparent border-none outline-none px-2.5 py-1 text-xs sm:text-sm font-sans resize-none max-h-36 min-h-[32px] overflow-y-auto leading-relaxed ${
            ...
            <button
              ...
              className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 self-end mb-0.5 transition-all transform active:scale-95 disabled:opacity-30 disabled:scale-100 disabled:shadow-none cursor-pointer shadow-md ${
                isDark
                  ? 'bg-brand-400 hover:bg-brand-300 text-black shadow-[0_0_12px_rgba(34,211,238,0.35)]'
          ...
          {/* Subtext info */}
          <div
            className={`flex items-center justify-between text-xs sm:text-xs px-2 font-mono ${
              isDark ? 'text-white/40' : 'text-slate-400'
            }`}
          >
            ...
            <button
              ...
              className={`flex items-center gap-1 cursor-pointer shrink-0 font-medium ${
                isDark ? 'text-brand-400 hover:underline' : 'text-brand-600 hover:underline'
              }`}
            >
