...
export const NeuralChat: React.FC<NeuralChatProps> = ({ messages, isThinking, onSendMessage, onOpenCLI, telemetry, onQuickPrompt, onClearChat, theme = 'dark' }) => {
  ...
  const toggleSpeechRecognition = () => {
    ...
    const recognition = new SpeechRec(); recognition.lang = 'hi-IN'; recognition.interimResults = false;
    ...
    <div ref={messagesContainerRef} className="flex-1 min-h-0 overflow-y-auto px-3 sm:px-6 py-4 space-y-4 overscroll-contain neural-messages-scroll">
      ...
      <div className="max-w-3xl mx-auto space-y-4 w-full pb-2">
        <div className={`flex items-center justify-between pb-2 border-b text-xs font-mono ${isDark?'border-white/5 text-white/40':'border-slate-200 text-slate-400'}`}>
          <span className="flex items-center gap-1.5 font-medium"><span className="w-2 h-2 rounded-full bg-brand-400 animate-pulse"/><span>NEURAL THREAD ACTIVE</span></span>
          {onClearChat && <button onClick={onClearChat} className={`flex items-center gap-1 transition px-2.5 py-1 rounded-lg ${isDark?'hover:text-brand-300 hover:bg-white/5':'hover:text-brand-600 hover:bg-slate-100 text-slate-500'}`}><RotateCcw className="w-3 h-3"/>Reset Thread</button>}
        </div>
        {messages.map(msg => { const jarvis=msg.sender==='jarvis', trace=msg.traceLog, open=expandedTraceId===msg.id; return <motion.div key={msg.id} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} className={`flex gap-2.5 sm:gap-3 max-w-full ${jarvis?'mr-auto justify-start':'ml-auto justify-end'}`}>
          ...
          <div className={`flex flex-col gap-1.5 max-w-[88%] sm:max-w-[80%] ${!jarvis?'items-end':'items-start'}`}>
            <div className={`p-3.5 sm:p-4 rounded-2xl text-xs sm:text-sm leading-relaxed shadow-sm font-sans break-words ${jarvis?(isDark?'bg-[#0f121d] border border-white/10 text-white/90 rounded-tl-sm':'bg-white border border-slate-200 text-slate-800 rounded-tl-sm'):(isDark?'bg-[#152336] border border-brand-500/40 text-brand-50 rounded-tr-sm':'bg-slate-900 text-white rounded-tr-sm')}`}>
              <p className="whitespace-pre-wrap break-words leading-relaxed">{msg.text}</p>
              {msg.extractedFact && <div className={`mt-2.5 pt-2 border-t flex items-start gap-1.5 text-xs font-mono p-2 rounded-xl ${isDark?'border-brand-400/30 bg-brand-400/10 text-brand-200':'border-sky-200 bg-sky-50 text-sky-800'}`}><Zap className="w-3.5 h-3.5 text-brand-400 shrink-0"/><span>Engram: <strong>{msg.extractedFact.subject}</strong> ➔ <em>{msg.extractedFact.predicate}</em> ➔ <u>{msg.extractedFact.value}</u></span></div>}
              {jarvis && <div className={`mt-2.5 flex items-center justify-between text-xs pt-1.5 border-t font-mono ${isDark?'border-white/5 text-white/40':'border-slate-100 text-slate-400'}`}><span>{msg.timestamp}</span><button onClick={()=>speakText(msg.text)} className="p-1 rounded-lg" title="Play voice"><Volume2 className="w-3.5 h-3.5"/></button></div>}
            </div>
            {jarvis && trace && <div className={`w-full border rounded-xl overflow-hidden font-mono text-xs ${isDark?'bg-black/40 border-white/10':'bg-slate-50 border-slate-200'}`}>
              <button onClick={()=>setExpandedTraceId(open?null:msg.id)} className={`w-full flex items-center justify-between px-3 py-1.5 ${isDark?'bg-white/[0.02] text-brand-300':'bg-white text-brand-700'}`}><span className="flex items-center gap-1.5"><Cpu className="w-3 h-3"/>Cognitive Trace <span className="text-white/40">({trace.latencySeconds.toFixed(2)}s)</span></span>{open?<ChevronUp className="w-3 h-3"/>:<ChevronDown className="w-3 h-3"/>}</button>
              <AnimatePresence>{open && <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} className={`p-3 space-y-2 border-t ${isDark?'border-white/10 text-white/80':'border-slate-200 text-slate-700'}`}>
                <div className="grid grid-cols-2 gap-2 pb-1.5 border-b"><span className="flex items-center gap-1"><Database className="w-2.5 h-2.5"/>FAISS: {trace.memoryLookupSeconds.toFixed(3)}s</span><span className="flex items-center gap-1"><Clock className="w-2.5 h-2.5"/>Inference: {trace.llmInferenceSeconds.toFixed(2)}s</span></div>
                {trace.typosCorrected?.length>0 && <div className="flex gap-1"><SpellCheck className="w-3 h-3"/>Normalized: {trace.typosCorrected.map((t,i)=><span key={i}>"{t.raw}" ➔ "{t.corrected}"</span>)}</div>}
                <div><div className="font-bold flex items-center gap-1"><Database className="w-2.5 h-2.5"/>Vector Matches ({trace.vectorMatches.length})</div>{trace.vectorMatches.map((m,i)=><div key={i} className="pl-2.5">{m.subject} ➔ {m.predicate} ➔ {String(m.value)} ({(m.similarity*100).toFixed(0)}%)</div>)}</div>
                <div><div className="font-bold flex items-center gap-1"><GitFork className="w-2.5 h-2.5"/>Graph Relations ({trace.graphRelations.length})</div>{trace.graphRelations.map((g,i)=><div key={i} className="pl-2.5">({g.subject}) ──[{g.predicate}]──&gt; ({g.target})</div>)}</div>
                <div className="pt-1 flex justify-between"><span className="flex items-center gap-1 text-green-500"><CheckCircle2 className="w-2.5 h-2.5"/>Pipeline Validated</span><span>{trace.traceId}</span></div>
              </motion.div>}</AnimatePresence>
            </div>}
          </div>
          {!jarvis && <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5 ${isDark?'bg-white/10 border border-white/20 text-white/80':'bg-slate-200 border border-slate-300 text-slate-700'}`}><User className="w-3.5 h-3.5"/></div>}
        </motion.div>; })}
