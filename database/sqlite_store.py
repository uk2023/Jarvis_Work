    ...
    def close(self) -> None:
        ...
        with self._lock:
            ...
            if self._connection is not None:
                ...
                # Previously this just closed the connection with no
                # explicit checkpoint. SQLite normally reclaims WAL space
                # on the last connection's close, but this method was
                # never actually called anywhere (see bootstrap.py
                # stop_jarvis) -- a session ending via Ctrl+C skipped
                # this entirely, so the WAL just kept accumulating across
                # every run instead of being flushed back into the main
                # database file.
                try:
